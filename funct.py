...
stock_derv=75.0  #1- Stocks/Futures , 75- Nifty options,etc.,
...
def Expectancy(alltrades,PLP,PL,n):
 i=0
 wintrades=losstrades=totaltrades=profper=lossper=winprob=losprob=profamt=lossamt=0.0
 for i in range(0,n):
  if alltrades[i]==1: 
   wintrades+=1
...
def PositionSizing_Method(postype,AccountSize,stop_loss,EntryPrice,Volatility,Uvolt,Dvolt,alltrades,PL,iter):
 Volatility=2.5*Volatility
 if iter<=10000:qty,StopPrice=PS_Per(postype,AccountSize,stop_loss,EntryPrice,Uvolt,Dvolt)
